...
let newCols = [
  '_ULV_STATUS_', // Validation status, possible values defined in loincUnitValidator.ValidationStatus
  '_ULV_NOTE_',  // validation note, if any, either empty or using values from loincUnitValidator.ValidationNotes
  '_ULV_SUGGEST_UNIT_', // if the input unit is not a valid UCUM unit but has a known mapping to a
                        // ucum unit that is compatible/valid to go with the input LOINC, then this column
                        // has that ucum unit, and the _ULV_STATUS_ column is set to be VALID.
].reduce((acc, status)=>{acc[status] = status; return acc;}, {});

const fs = require('fs');
const csv = require('csv');
const validator = require('../source/loincUnitValidator');
...
function validateRecord(record, loincCol, unitCol) {
  ...
  if(record[loincCol] && record[unitCol]) {
    let status = validator.validate(record[loincCol], record[unitCol]);
    record._ULV_STATUS_ = status.status;
    record._ULV_SUGGEST_UNIT_ = status.suggested_unit;
    record._ULV_NOTE_ = status.note;
  }
  else { // add empty value to the new columns
    Object.keys(newCols).forEach(key => {record[key] = ''});
  }
